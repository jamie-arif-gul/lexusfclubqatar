    ...
    <div class="col-sm-12 padding0">
      ...
      <div class="clear10"></div>
        ...
        <?php if($result['user_id'] != $this->session->userdata('user_id')){ ?>
                <!-- message us start-->
                <button type="button" class="btn btn-success" data-toggle="modal" data-target="#messageUsModel"><i class="fa fa-envelope"></i> Message Us</button>
                <div id="messageUsModel" class="modal fade" role="dialog">
                  <div class="modal-dialog"> 
                    ...
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        ...
                        <form id="messageUsForm" action="" method="post" accept-charset="utf-8">
                          ...
                          <div class="col-sm-6" style="padding-right:0;">
                              ...
                              <input type="text" class="form-control" name="last_name" id="last_name" value="" placeholder="Last">
                          </div>
                          ...
                          <div class="form-group">
                            ...
                            <textarea class="form-control" name="message" id="message" placeholder="Have a question about the property? Message us and we will get you the answers."></textarea>
                          </div>
                        
                        </form>
                      </div>
                      <div class="modal-footer">
                        <button type="button" class="btn btn-danger" onclick="submitUs()" >Send</button>
                      </div>
                    </div>
                  </div>
                </div>
                <script type="text/javascript">
                  ...
                  //alert(p_id+'--'+p_review);
                      $.ajax({
                        url: jqv_ajax_url+'message_us', //your server side script
                        type: 'POST',
                        dataType : "json",
                        //contentType: "application/json; charset=utf-8",
                        dataType: "text",
                        data : { "name": name, 'last_name':last_name, "email": email,'message' : message },
                        success: function (result){
                          if(result == 'done'){
                            $('#status_message').html('<div style="color:green;">Your message has beed sent.</div>');
                            $('#status_message').show();
                            $("#messageUsForm")[0].reset()
                            setTimeout(function() {
                              $('#messageUsModel').modal('hide');
                            }, 2000);
                            $('#status_message').html('');
                            $('#status_message').hide();
                          }else{
                            //alert(result);
                            $('#status_message').html('<div style="color:red;">'+result+'</div>');
                            $('#status_message').show();
                          }
                         }
                     }); 
        ...
        <?php
            $is_rented = $this->comman_model->get('request_property', array('property_id' => $result['property_id'],'check_out_date > '=> time(),'request_status'=> 1));
            if($is_rented){ ?>
